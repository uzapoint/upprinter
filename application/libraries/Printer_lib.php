...
class Printer_lib
{
    ...
    public function captainMinifiedDesign($request = array())
    {
        ...
        $header = sprintf("%-29s %-3s %-6s", "Item", "Qty", "Total");
        ...
        foreach ($request['items'] as $item) {
            ...
            $numbers = strlen(sprintf("    %-5s %-6s",$item['qty'], number_format((float)$item['total'], 2)));
            $myItem = sprintf("%-29s %-5s %-6s", substr($item['item_name_only'], 0, 42-($numbers)), $item['qty'], number_format((float)$item['total'], 2));
