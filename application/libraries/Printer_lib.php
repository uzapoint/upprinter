...
class Printer_lib
{
    ...
    public function captainMinifiedDesign($request = array())
    {
        ...
        // $header = sprintf("%-29s %-5s %-6s", "Item", "Qty", "Total");
        $header = sprintf("%-29s %-5s %-6s", "Item", "Qty", "Total");
        ...
        foreach ($request['items'] as $item) {
            ...
            // $myItem = sprintf("%-29s %-3s %-8s", substr($item['item_name_only'], 0, 42-($numbers)), $item['qty'], number_format((float)$item['total'], 2));
            $numbers = (sprintf("%3s  %s",$item['qty'], number_format((float)$item['total'], 2)));
            $myItem = sprintf("%s   %s  %s", substr($item['item_name_only'], 0, (42-strlen($numbers))), $item['qty'], number_format((float)$item['total'], 2));
